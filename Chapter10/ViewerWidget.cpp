...
void ViewerWidget::paintEvent(QPaintEvent* /*event*/) {
  ...
  for (const Action& action : m_actionList) {
    switch (action.id()) {
      ...
      case SetScaleId:
        painter.scale(action.value1().numericalValue(),
                      action.value2().numericalValue());
        break;

      case SetRotateId:
        painter.rotate(action.value1().numericalValue());
        break;

      case SetOriginId:
        painter.translate(action.value1().pointValue());
        break;
